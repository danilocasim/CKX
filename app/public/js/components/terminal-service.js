...
// Runtime isolation: terminalSessionId (UUID) + examId + token; SSH is keyed by terminalSessionId only
...
// Set context so /ssh attaches only to this user's runtime (terminalSessionId = terminal_session.id)
function setConnectionContext(examId, terminalSessionId, getToken) {
  connectionContext = {
    examId: examId || null,
...
function resizeTerminal(containerElement) {
  ...
  setTimeout(() => {
    ...
    if (fitAddon) {
      fitAddon.fit();
      console.log(
        `Terminal resized to ${terminal.cols} columns by ${terminal.rows} rows`
      );
...
function connectToSocketIO() {
  ...
  const terminalSessionId = connectionContext.terminalSessionId;
  const token = connectionContext.getToken
    ? connectionContext.getToken()
    : null;
  if (!terminalSessionId || !examId || !token) {
    console.error(
      'Terminal requires terminalSessionId, examId, and token. Set via setConnectionContext(examId, terminalSessionId, getToken).'
    );
    if (terminal) {
      terminal.writeln(
        '\r\n\x1b[1;31m[ERROR]\x1b[0m Terminal requires session context. Refresh or reopen the terminal.\r\n'
      );
    }
    return;
  }
  ...
  socket = io('/ssh', {
    ...
    timeout: 1000,
    transports: ['polling'],
    query: { terminalSessionId, examId, token },
    auth: { terminalSessionId, examId, token },
  });
  console.log(
    'Connecting to SSH runtime for terminal session',
    terminalSessionId
  );
  ...
  socket.on('connect', () => {
    ...
    if (terminalCallbacks.showConnectionStatus) {
      terminalCallbacks.showConnectionStatus(
  ...
  socket.on('disconnect', () => {
    ...
    if (terminalCallbacks.showConnectionStatus) {
      terminalCallbacks.showConnectionStatus(
        'Disconnected from terminal. Reconnecting...',
        'error'
      );
    }
