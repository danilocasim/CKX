...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const terminateModal = new bootstrap.Modal(
    document.getElementById('terminateModal')
  );
  const startExamModal = new bootstrap.Modal(
    document.getElementById('startExamModal')
  );
  const examEndModal = new bootstrap.Modal(
    document.getElementById('examEndModal')
  );
  ...
  document.addEventListener('examCompletedSession', function (event) {
    ...
    Promise.all(promises)
      .then(([examInfoData, questionsData]) => {
        ...
        const endExamItem = document.querySelector(
          '.dropdown-item[href="#"][id="endExamBtnDropdown"]'
        );
        if (endExamItem) {
  ...
  function connectToExamSession() {
    ...
    RemoteDesktopService.setupRemoteDesktopFrameHandlers(
      vncFrame,
      showVncConnectionStatus
    );
      ...
      .then((status) => {
        ...
        return ExamApi.fetchCurrentExamInfo()
          .then((data) => {
            ...
            if (data.info) {
              if (
                data.info.events?.examStartTime &&
                data.info.examDurationInMinutes
              ) {
                // Calculate remaining time for in-progress exams
                const remainingMinutes =
                  TimerService.calculateRemainingTime(data);
                console.log(
                  `Setting timer to ${remainingMinutes} minutes (based on start time)`
                );
                TimerService.setTimerDuration(remainingMinutes);
              } else if (data.info.examDurationInMinutes) {
                // For new exams, use the full duration
                console.log(
                  `Setting timer to ${data.info.examDurationInMinutes} minutes`
                );
                TimerService.setTimerDuration(data.info.examDurationInMinutes);
              } else {
                console.warn(
                  'examDurationInMinutes not found in API response, using default duration'
                );
              }
            }
  ...
  function setupNewExamButton() {
    ...
    // Reset modal title
    document.getElementById('startExamModalLabel').textContent =
  ...
  function startExam() {
    ...
    WakeLockService.acquireWakeLock().then((success) => {
      if (success) {
  ...
  function handleExamEnd() {
    ...
    WakeLockService.releaseWakeLock().then((success) => {
      if (success) {
    ...
    trackExamEndEvent()
      ...
      .catch((error) => {
        ...
        alert(
          'There was an error evaluating your exam. Please try manually ending the exam.'
        );
      });
  }
  ...
  function updateQuestionContent(questionId) {
    const question = questions.find(
      (q) => q.id === questionId || q.id === questionId.toString()
    );

    if (!question) {
      console.error(`Question with ID ${questionId} not found`);
      questionContent.innerHTML =
        '<div class="alert alert-danger">Question not found. Please try another question.</div>';
      return;
    }
      ...
      setTimeout(() => {
        try {
          ...
          const formattedContent =
            QuestionService.generateQuestionContent(question);
              ...
              nextQuestionBtn.addEventListener('click', function () {
                const currentIndex = questions.findIndex(
                  (q) =>
                    ...
                    q.id === currentQuestionId.toString()
                );
          ...
          questionContent.innerHTML =
            '<div class="alert alert-danger">Error displaying question content. Please try refreshing the page.</div>';
          questionContent.classList.remove('content-fade');
        }
      }, 100);
    });
  ...
  function toggleView() {
    ...
    if (isTerminalActive) {
      ...
      UiUtils.showToast(
        'Note: In the actual certification exam, only remote desktop access is available. Terminal access is provided here for practice convenience.',
        {
          bgColor: 'bg-info',
          textColor: 'text-white',
          delay: 8000,
      ...
      (async function () {
        ...
        TerminalService.setConnectionContext(examId, session.id, () =>
    ...
    if (isCompletedExamMode) {
      ...
      if (questionContent) {
        ...
        reviewBanner.textContent =
          'Review Mode: This exam has been completed. You can review questions and your environment.';
  ...
  function setupUIEventListeners() {
    ...
    const fullscreenTerminalBtn = document.getElementById(
      'fullscreenTerminalBtn'
    );
    ...
    document.addEventListener('fullscreenchange', () =>
      UiUtils.updateFullscreenUI(fullscreenBtn)
    );
  ...
  prevBtn.addEventListener('click', () => {
    const currentIndex = questions.findIndex(
      (q) => q.id === currentQuestionId || q.id === currentQuestionId.toString()
    );
    if (currentIndex > 0) {
      currentQuestionId = questions[currentIndex - 1].id;
      updateQuestionContent(currentQuestionId);
  ...
  confirmEndBtn.addEventListener('click', () => {
    ...
    trackExamEndEvent()
      ...
      .catch((error) => {
        ...
        pageLoader.style.display = 'none';
        alert(
          'There was an error ending your exam. Please try again or contact support.'
        );
      });
  });
  ...
  confirmTerminateBtn.addEventListener('click', () => {
    ...
    ExamApi.terminateSession(examId)
      ...
      .catch((error) => {
        ...
        alert(
          'Failed to terminate session. Please try again or contact support.'
        );
        // Hide loader
        pageLoader.style.display = 'none';
      });
  ...
  function updateQuestionDropdown() {
    QuestionService.updateQuestionDropdown(
      questions,
