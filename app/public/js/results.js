...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const totalPossibleScoreElement =
    document.getElementById('totalPossibleScore');
  ...
  confirmTerminateBtn.addEventListener('click', function () {
    ...
    confirmTerminateBtn.innerHTML =
      '<i class="fas fa-spinner fa-spin me-2"></i> Terminating...';

    // Call the API to terminate the session
    apiFetch(`/sailor-client/api/v1/exams/${currentExamId}/terminate`, {
      method: 'POST',
      headers: {
  ...
  function initiateReEvaluation(examId) {
    ...
    apiFetch(`/sailor-client/api/v1/exams/${examId}/evaluate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  function startPolling(examId) {
    ...
    updateLoaderMessage(
      'Evaluation in progress... (typically takes 2-3 minutes)'
    );
  ...
  function checkExamStatus(examId) {
    ...
    if (Date.now() - pollingStartTime > MAX_POLLING_TIME) {
      stopPolling();
      showError(
        'Exam evaluation is taking longer than expected. Please try again later.'
      );
      return;
    }
    ...
    apiFetch(`/sailor-client/api/v1/exams/${examId}/status`)
      .then((response) => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
      ...
      .then((data) => {
        ...
        if (data.status === 'EVALUATED') {
          ...
          updateLoaderMessage(
            `Waiting for evaluation to start... Current status: ${data.status}`
          );
        }
      })
      .catch((error) => {
  ...
  function fetchExamResults(examId = null) {
    ...
    if (!examId) {
      ...
      if (!examId) {
        showError(
          'No exam ID provided. Please return to the dashboard and try again.'
        );
        return;
      }
    }
    ...
    const apiUrl = `/sailor-client/api/v1/exams/${examId}/result`;
    ...
    return apiFetch(`/facilitator/api/v1/exams/${examId}/status`)
      ...
      .then((data) => {
        if (data.status === 'EVALUATING') {
          ...
          throw new Error(
            `Exam results not available. Current status: ${data.status}`
          );
        } else {
  ...
  function renderExamResults(results) {
    ...
    completedAtElement.textContent = `Completed: ${formatDate(
      results.completedAt
    )}`;
    ...
    if (results.evaluationResults && results.evaluationResults.length > 0) {
      ...
      sortedQuestions.forEach((question) => {
        ...
        const sortedVerifications = [...question.verificationResults].sort(
          (a, b) => {
            const idA = parseInt(a.id);
            const idB = parseInt(b.id);
            return idA - idB;
        ...
        const questionScore = sortedVerifications.reduce(
          (total, v) => total + v.score,
          0
        );
        const questionPossibleScore = sortedVerifications.reduce(
          (total, v) => total + v.weightage,
          0
        );
